...
void Signature::make()
{
    ...
    (mpz_divisible_p(hash, attributes.q)) ? mpz_set_ui(h, 1u) : mpz_set(h, hash);
                       ...
                       const __mpz_struct *hash)
{
    ...
    mpz_mod(u, u, p);
    mpz_mod(u, u, q);
    // std::cout << "r, u for verify:" << std::endl;
    // gmp_printf("r: %Zd \n", r);
    // gmp_printf("u: %Zd \n", u);
...
void Signature::Attributes::generate(const mpz_t pass, bool random)
{
    if (random)
    {
        generatePrimes1024();
